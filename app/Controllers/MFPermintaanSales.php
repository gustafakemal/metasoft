...
class MFPermintaanSales extends BaseController
{
	private $model;
	private $errors = [];
	...
	public function index()
	{
		...
		return view('MFPermintaanSales/cari', [
	...
	public function input($id)
	{
		...
		$khusus_model = new \App\Models\MFProsesKhususModel();
		//die($id);
		$data = $this->model->getByIDProduk($id);
		$data_warnabackside =  $this->model->getWarnaById($id,'B');
		$data_warnafrontside =  $this->model->getWarnaById($id,'F');
		return view('MFPermintaanSales/input', [
			'page_title' => 'Permintaan Sales',
			'id_produk' => $id,
			'data_produk' => $data,
			'data_backside' => $data_warnabackside,
			'data_frontside' => $data_warnafrontside,
			'opsi_segmen' => $segmen_model->getAll(),
			'opsi_customer' => $customer_model->getOpsi(),
			'opsi_sales' => $sales_model->getOpsi(),
			// 'opsi_sales' => [],
	...
	public function productSearch()
	{
		...
		foreach($query as $key => $val) {
			$input_btn = '<a type="button" class="btn btn-sm btn-success input-permintaansales-item mr-2" data-id="'.$val->id.'" href="mfpermintaansales/input/'.$val->id.'">Permintaan</a>';
	...
	{
		if ($this->request->getMethod() !== 'post') {
			return redirect()->to('mfpermintaansales');
		}
