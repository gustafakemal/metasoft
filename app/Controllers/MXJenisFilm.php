...
class MXJenisFilm extends BaseController
{
    ...
    public function apiGetAll(): ResponseInterface
    {
        ...
        foreach ($query as $key => $value) {
            ...
            $edit = '<a class="btn btn-success btn-sm item-edit mr-1" href="#" data-id="' . $value->id . '" data-nama="' . $value->nama . '" data-beratjenis="' . $value->berat_jenis . '" data-harga="' . $value->harga . '" data-added="' . $CreateDate . '" data-aktif="' . $value->aktif . '|Y,T" title="Edit"><i class="far fa-edit"></i></a>';
            $hapus = '<a class="btn btn-danger btn-sm" href="' . site_url('mxjenisfilm/delete/' . $value->id) . '" data-id="' . $value->id . '" onclick="return confirm(\'Apa Anda yakin menghapus data ini?\')" title="Hapus"><i class="fas fa-trash-alt"></i></a>';
