...
use App\Models\MXJenisProdukModel;
use CodeIgniter\HTTP\RedirectResponse;
use CodeIgniter\HTTP\ResponseInterface;
use CodeIgniter\I18n\Time;

class MXJenisProduk extends BaseController
{
    private $model;

    public function __construct()
    {
        $this->model = new MXJenisProdukModel();
    }
    ...
    public function index(): string
    {
        ...
        $this->breadcrumbs->add('Data Jenis Produk', '/mxjenisproduk');

        return view('MXJenisProduk/main', [
            'page_title' => 'Data Jenis Produk',
    ...
    {
        $query = $this->model->getMXJenisProduk();
        ...
        foreach ($query as $key => $value) {
            ...
            $edit = '<a class="btn btn-success btn-sm item-edit mr-1" href="#" data-id="' . $value->id . '" data-nama="' . $value->nama . '" data-added="' . $CreateDate . '" data-aktif="' . $value->aktif . '|Y,T" title="Edit"><i class="far fa-edit"></i></a>';
            $hapus = '<a class="btn btn-danger btn-sm" href="' . site_url('mxjenisproduk/delete/' . $value->id) . '" data-id="' . $value->id . '" onclick="return confirm(\'Apa Anda yakin menghapus data ini?\')" title="Hapus"><i class="fas fa-trash-alt"></i></a>';
