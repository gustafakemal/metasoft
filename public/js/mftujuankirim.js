...
$(function () {
	...
	const config = {
		...
		createdRow: ['No', 'Tanggal dibuat', 'Tujuan Kirim', 'Harga', 'Status Aktif', 'Action'],
	...
	const datatable = new Datatable('#dataList', config, `${HOST}/tujuankirim/api`, 'GET')
	datatable.load()
	...
	$('#dataList').on('click', '.item-edit', function(e) {
		...
		const id = $(this).attr('data-id');
		const tujuan = $(this).attr('data-tujuan');
		...
		$(`#dataList tr:nth-child(${row}) td:nth-child(3)`).html(`<input type="text" class="form-control" placeholder="Tujuan Kirim" value="${tujuan}" name="tujuan" />`)
