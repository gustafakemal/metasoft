...
$(function () {
	...
	const datatable = new Datatable('#dataList', config, `${HOST}/mxjenisfilm/api`, 'GET')
	datatable.load()
	...
	$('#dataList').on('click', '.item-edit', function(e) {
		...
		$(`#dataList tr:nth-child(${row}) td:nth-child(3)`).html(`<input type="text" class="form-control" placeholder="Jenis film" value="${nama}" name="nama" />`)
		$(`#dataList tr:nth-child(${row}) td:nth-child(4)`).html(`<input type="number" class="form-control" placeholder="Berat Jenis" value="${parseInt(beratjenis)}" name="berat_jenis" />`)
	...
	$('#dataList').on('click', '.cancel-tr-submit', reload_tr)
		...
		.on('click', '.save-tr-record', function() {
			const data = {
				...
				berat_jenis: $('input[name="berat_jenis"]').val(),
				harga: $('input[name="harga"]').val(),
				aktif: $('select[name="aktif"] option:selected').val()
			};
			$.ajax({
				type: "PUT",
				url: `${HOST}/mxjenisfilm/edit/api`,
