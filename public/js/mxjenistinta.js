...
$(function () {
	...
	const datatable = new Datatable('#dataList', config, `${HOST}/mxjenistinta/api`, 'GET')
	datatable.load()
	...
	$('#dataList').on('click', '.item-edit', function(e) {
		...
		const merk_arr = $(this).attr('data-merk').split('|');
		const merk_opt_arr = merk_arr[1].split(',');
		const merk_opt = [];
		merk_opt.push(`<option value=""></option>`);
		for(let j = 0;j < merk_opt_arr.length; j++) {
			merk_opt.push(`<option${merk_opt_arr[j] == merk_arr[0] ? ' selected' : ''} value="${merk_opt_arr[j]}">${merk_opt_arr[j].toUpperCase()}</option>`);
		}
		const merk = `<select name="merk" class="form-control">${merk_opt.join('')}</select>`
	...
	$('#dataList').on('click', '.cancel-tr-submit', reload_tr)
		...
		.on('click', '.save-tr-record', function() {
			...
			$.ajax({
				type: "PUT",
				url: `${HOST}/mxjenistinta/edit/api`,
