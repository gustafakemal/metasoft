...
$(function () {
	...
	const config = {
		...
		createdRow: ['No', 'Tanggal dibuat', 'Solvent Tinta', 'Harga', 'Status Aktif', 'Action'],
	...
	const datatable = new Datatable('#dataList', config, `${HOST}/mxsolventtinta/api`, 'GET')
	datatable.load()
	...
	$('#dataForm').on('submit', 'form[name="addData"]', function(e) {
		...
		$.ajax({
			type: "POST",
			url: `${HOST}/mxsolventtinta/add/api`,
			dataType: 'JSON',
			data: formData,
			contentType: false,
			processData: false,
	...
	$('#dataList').on('click', '.item-detail', function(e) {
		...
		$.ajax({
			type: "GET",
			url: `${HOST}/mxsolventtinta/api/${id}`,
			dataType: 'JSON',
			beforeSend: function () {},
			success: function (response) {
				if(response.success) {
	...
	$('#dataList').on('click', '.item-edit', function(e) {
		...
		$(`#dataList tr:nth-child(${row}) td:nth-child(3)`).html(`<input type="text" class="form-control" placeholder="Tinta Solvent" value="${nama}" name="nama" />`)
	...
	$('#dataList').on('click', '.save-tr-record', function() {
		...
		$.ajax({
			type: "PUT",
			url: `${HOST}/mxsolventtinta/edit/api`,
	...
	$('#dataForm').on('submit', 'form[name="editData"]', function(e) {
		...
		$.ajax({
			type: "POST",
			url: `${HOST}/mxsolventtinta/apiEditProcess`,
			dataType: 'JSON',
			data: formData,
			contentType: false,
			processData: false,
